namespace Tekken101.Characters
{
    ...
    public class Lili : BaseAssassin
    {
        ...
        private BaseSkill attackOne = new BaseSkill(SpriteType.DevotionKiss, "Devotion Kiss", DestriptionSkillOne, 55, 50, 70, 200);
        private BaseSkill attackTwo = new BaseSkill(SpriteType.SoulSweep, "Soul Sweep", DestriptionSkillTwo, 55, 50, 70, 200);
        private BaseSkill attackThree = new BaseSkill(SpriteType.JudgmentSignal, "Judgment Signal", DestriptionSkillThree, 55, 50, 70, 200);
        private BaseSkill attackFour = new BaseSkill(SpriteType.KittyClaws, "Kitty Claws", DestriptionSkillFour, 55, 50, 70, 200);

        public Lili(SpriteType type, int X, string name, string description, int initialHealth, int initialSpeed, int initialDefense, int initialCriticalChance, int initialLifeSteal)
            : base(X, name, description, initialHealth, initialSpeed, initialDefense, initialCriticalChance, initialLifeSteal)
        {
            this.SpriteType = type;
            this.PlayerType = PlayerType.Lili;
        }
